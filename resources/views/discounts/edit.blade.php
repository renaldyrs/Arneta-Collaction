...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Edit Diskon</h1>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        <form action="{{ route('discounts.update', $discount) }}" method="POST" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="min_purchase" value="{{ old('min_purchase', $discount->min_purchase) }}" min="0" step="1000"
                ...
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date', $discount->start_date?->format('Y-m-d')) }}"
                        class="w-full px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tanggal Berakhir</label>
                    <input type="date" name="end_date" value="{{ old('end_date', $discount->end_date?->format('Y-m-d')) }}"
                        class="w-full px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm">
            ...
            <div class="flex items-center gap-3 pt-1">
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $discount->is_active ? 'checked' : '' }}>
...
function updateValueLabel() {
    ...
    document.getElementById('valuePrefix').textContent = type === 'percentage' ? '%' : 'Rp';
}
updateValueLabel();
</script>
@endpush